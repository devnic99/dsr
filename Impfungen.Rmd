...
vacc = read.csv(url('https://impfdashboard.de/static/data/germany_vaccinations_timeseries_v2.tsv'), sep='\t')
vacc$date = as.Date(vacc$date, "%Y-%m-%d")
print(head(vacc, 3))

deliv = read.csv(url('https://impfdashboard.de/static/data/germany_deliveries_timeseries_v2.tsv'), sep='\t')
deliv$date = as.Date(deliv$date, "%Y-%m-%d")
print(head(deliv, 3))

vaccState = read.csv(url('https://impfdashboard.de/static/data/germany_vaccinations_by_state.tsv'), sep='\t')
...
vacc2 = vacc |> mutate(
  ...
  astra = dosen_astra_kumulativ  - lag(dosen_astra_kumulativ, 1, 0)) |> select(date, biontech, moderna, astra) |> pivot_longer(cols=c(2,3,4))
...
# Ihre Lösung:
ggplot(vacc2) +
  geom_bar(aes(x=date, y=value, group=name, color=name), stat='identity')
```
