...
expected_warning = 'Folgende Werte befinden sich außerhalb des vorgegebenen Bereichs:  6'
```
...
solution3 = list(borders = c(5, 14, 23, 32, 41, 50),
                 counts = c(2, 2, 2, 2, 1))
expected_warning = 'Folgende Werte befinden sich außerhalb des vorgegebenen Bereichs:  
30, 35, 40, 45'
```
...
myhistogram = function(x, n = length(x)-1, min = base::min(x), max = base::max(x)) {
  ...
  if (length(ignored_vals) > 0) {
    warn_msg = paste("Folgende Werte befinden sich außerhalb des vorgegebenen Bereichs: ", 
    paste(unlist(sort(ignored_vals)), collapse = ", "))
    warning(warn_msg)
  }
