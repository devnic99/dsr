...
```{r}
# Ihre Lösung:
herz = read.csv(url("https://oc.informatik.hs-mannheim.de/s/wyzFq34K9HiNjXR/download")) |> mutate(across(c("sex", "cp","fbs", "restecg", "exang", "slope", "goal"),as.factor))
...
```{r}
# Ihre Lösung:
korrekte_zuordnungen = sum(herz$goal == ifelse(kmeans_result$cluster[1] == 1, 0, 1)) # 2mal vergleichen
